...
Floresta Aleatória:
```{r}
# sugestão de RMD usando programação funcional:
...
# Função para treinamento, previsão e armazenamento de resultados
train_and_predict <- function(split, sarrafo) {
  ...
  y_pred_prob <- if (is.matrix(predictions$predictions)) predictions$predictions[, "1"] else predictions$predictions
  y_pred <- as.factor(ifelse(y_pred_prob > sarrafo, 1, 0))
  y_real <- as.factor(valid_data$operates_within_2_years)
  
  # Retornando os resultados como tibble
  tibble(
    truth = y_real,
    estimate = y_pred,
    .pred_1 = y_pred_prob
...
# Aplicando a função a todos os splits
results_list <- map(folds$splits, ~ train_and_predict(.x, sarrafo))
...
# Calculando e armazenando o ROC
roc_forest <- calcular_roc(resultados_cv$truth, resultados_cv$.pred_1, "Floresta Aleatória")
